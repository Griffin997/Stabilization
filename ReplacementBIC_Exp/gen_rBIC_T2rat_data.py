...
# It is important to note that a zero corresponds to BiX and a 1 corresponds to MoX
...
data_path = "ReplacementBIC_Exp/rBIC_DATA"
add_tag = "standard"
data_tag = (f"rBIC_{add_tag}_T2rat_SNRsuite_{day}{month}{year}")
...
def convert_list(binary_list):
    curve_options = ["BiX", "MoX"] 
    label_list = [curve_options[int(elem)] for elem in binary_list]
    return label_list
...
def estimate_parameters(popt, TI_DATA, noised_data, lb, ub, SNR, list_curve_X, list_curve_cvn):

    bnds = bounds_condensed(lb, ub)

    BIC_list_comp = create_BIC_list(TI_DATA, noised_data, SNR)
    list_curve_BIC = list_curve_X*BIC_list_comp

    res_COFFEE = minimize(list_objective_func, popt, args = (noised_data, TI_DATA, list_curve_BIC), method = 'Nelder-Mead', bounds = bnds, options = {'maxiter': 4000, 'disp': False})
...
def generate_all_estimates(i_param_combo):
    ...
    for nr in range(var_reps):    #Loop through all noise realizations
        ...
        param_est_COFFEE[nr,:], param_est_cvn[nr,:] = estimate_parameters(param_est_cF[nr,:], TI_NP, noised_data, lb, ub, SNR_eTime, Exp_NP, Exp_STANDARD)
