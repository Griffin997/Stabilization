...
def generate_all_estimates(i_param_combo):
    ...
    TI_combo = target_iterator[i_param_combo][0:2]
    alpha = target_iterator[i_param_combo][2]
    TI_DATA = [0, TI_combo[0], TI_combo[1]]
    feature_df = pd.DataFrame(columns = ["TI_combo","alpha","MSE", "Var", "Bias", "AIC"])

    feature_df["TI_combo"] = [TI_combo]
    feature_df["alpha"] = alpha
    ...
    for nr in range(var_reps):    #Loop through all noise realizations
        ...
        for iter in range(noised_data.shape[0]):
            mox_opt_iter, _, _ = evaluate_model(noised_data[iter,:], alpha)
            AIC_moX_analysis[iter] += mox_opt_iter
            list_curve_AIC.append(curve_options[mox_opt_iter])
            list_curve_cvn.append(curve_options[0])

        param_est_AIC[nr,:], param_est_cvn[nr,:], param_est_cF[nr,:] = estimate_parameters(TE_DATA, TI_DATA, noised_data, lb, ub, list_curve_AIC, list_curve_cvn)
...
hprParams = {
    "SNR_value": SNR_value,
    'TI1_region': TI1_array,
    'TI2_region': TI2_array,
    'alpha_region': alpha_array,
