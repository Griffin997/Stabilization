...
target_iterator = [c for c in SNR_values]
...
def generate_all_estimates(i_param_combo):
    ...
    SNR_value = target_iterator[i_param_combo]

    feature_df = pd.DataFrame(columns = ["SNR","MSE", "Var", "bias", "AIC", "pEst_AIC", "pEst_cf"])
    ...
    for iTI in range(len(TI_STANDARD)):
        signal_array[iTI,:] = S_biX_6p(TE_DATA, *true_params, TI = TI_STANDARD[iTI])
    ...
    for nr in range(var_reps):    #Loop through all noise realizations
        ...
        param_est_AIC[nr,:], param_est_cvn[nr,:], param_est_cF[nr,:] = estimate_parameters(TE_DATA, TI_STANDARD, noised_data, lb, ub, list_curve_AIC, list_curve_cvn)
