...
T11 = 600
T12_ratio = np.arange(1.1,2.6,0.1)
c1 = 0.4
c2 = 0.6 
T21 = 40
T22_ratio = np.arange(1.1,2.6,0.1)
...
TI_STANDARD = np.append(0,np.logspace(1,np.log10(3*normal_T12),11))//1

#Constrct Window Differences
rad_diff = 15
np_radius = np.array([-2, -1, 0 , 1, 2])*rad_diff

TI1star = np.log(2)*T11
# TI2star = np.log(2)*T12   This now becomes variable because of the ratios that we are looking at
...
target_iterator = [(a,b,c) for a in center_array for b in T12_ratio for c in T22_ratio]
...
def int_length_AIC(R0, diff_TE, SNR):
    return (2*diff_TE/(R0*SNR**2))**(1/2)

def evaluate_model(data, SNR_value, ms_iter = multi_starts_AIC):
...
def estimate_parameters(TE_DATA, TI_DATA, noised_data, lb, ub, list_curve_AIC, list_curve_cvn):
    ...
    res_AIC = minimize(list_objective_func, popt, args = (noised_data, TI_DATA, list_curve_AIC), method = 'Nelder-Mead', bounds = bnds, options = {'maxiter': 4000, 'disp': False})
...
def generate_all_estimates(i_param_combo):
    ...
    for iTI in range(len(TI_ESPRESSO)):
        signal_array[iTI,:] = S_biX_6p(TE_DATA, *true_params, TI = TI_ESPRESSO[iTI])

    #Temp Parameter matrices
    param_est_AIC = np.zeros((var_reps, len(true_params)))
    param_est_cvn = np.zeros((var_reps, len(true_params)))
    param_est_cF = np.zeros((var_reps, len(true_params)))

    AIC_moX_analysis = np.zeros((signal_array.shape[0]))
    ...
    for nr in range(var_reps):    #Loop through all noise realizations
        ...
        for iter in range(noised_data.shape[0]):
            mox_opt_iter, _, _ = evaluate_model(noised_data[iter,:], SNR_value)
            AIC_moX_analysis[iter] += mox_opt_iter
            list_curve_AIC.append(curve_options[mox_opt_iter])
            list_curve_cvn.append(curve_options[0])

        param_est_AIC[nr,:], param_est_cvn[nr,:], param_est_cF[nr,:] = estimate_parameters(TE_DATA, TI_ESPRESSO, noised_data, lb, ub, list_curve_AIC, list_curve_cvn)

    MSE_mat[0,:], var_mat[0,:], bias_mat[0,:] = calc_MSE(param_est_AIC, true_params)
